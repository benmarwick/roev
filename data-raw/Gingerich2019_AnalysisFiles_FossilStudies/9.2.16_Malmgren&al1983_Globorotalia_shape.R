...
##9.2.16_Malmgren&al1983_Globorotalia_shape
#==========================================================================
cat(rep("\n",50))							#clear console
...
for (i in seq(4,16,2)){
  text(i,yos,format(round((i-10)/40,digits=2),nsmall=2),
    pos=1,cex=1,col=1)
}
text(xos,yos+8.0,expression(paste(italic('Globorotalia tumida')~
	'lineage:  shape')),pos=4,cex=1.3,col=1)
text(xos-1.5,yos+4,'Geological age (Ma)',
	srt=90,cex=1.3,col=1)
text(xos+7.5,yos-1.2,
  expression(paste('2nd eigenshape')),cex=1.3,col=1)
#---------------------------------------------------------------- Plot ranges
LM[1:5,]
min(na.omit(LM[,6]));max(na.omit(LM[,6]))
min(LM[,1]);max(LM[,1])
xpos=18;ypos=25
for (i in 1:nrow(LM)){
  lines(xos+40*c(LM[i,6]-LM[i,7],LM[i,6]+LM[i,7])+8,
    ypos-c(yos-1+LM[i,1],yos-1+LM[i,1]),lty=1,lwd=1,col=gray(6/10))
}
lines(xos+40*LM[,6]+7,ypos-(yos-1+LM[,1]),lty=1,lwd=2,col=1)

text(15.8,12,expression(paste(italic('G. tumida'))),pos=4,cex=1,col=1)
text(11.6,10.2,expression(paste(italic('G. plesiotumida'))),pos=4,cex=1,col=1)
...
writefile<-"C://R_aaROEVchapt09//9.2.16_Malmgren&al1983_Globorotalia_shape_out.csv"
