...
##9.2.27_Bell&al2006_Gasterosteus_var2-3
#==========================================================================
cat(rep("\n",50))							#clear console
...
zc1=0							#zero count
nc=0
for (cn in seq(6,ncol(SP1),3)){		#columns of means
...
zct=0							#zero n count
nc=0
for (cn in seq(6,ncol(SPt),3)){		#columns of means
  for (i in 1:(nr-1)){				#increment
    for (sr in 1:(nr-i)){			#start row
      id<-numeric(length=9)
      nc=nc+1
...
zc2=0							#zero n count
nc=0
for (cn in seq(6,ncol(SP2),3)){		#columns of means
  for (i in 1:(nr-1)){				#increment
    for (sr in 1:(nr-i)){			#start row
      id<-numeric(length=9)
      nc=nc+1
...
writefile<-"C://R_aaROEVchapt09//9.2.27_Bell&al2006_Gasterosteus23_out.csv"
...
text(idrxt[2,4]+1,idrxt[2,6]+6.7,'highest rates',pos=4,cex=1,col=1)
points(idrxt[,4]+13,idrxt[,6]+6.5,pch=19,cex=1.1,col=1)
text(idrxt[2,4]+13,idrxt[2,6]+6.7,'highest rates',pos=4,cex=1,col=1)

text(9.4,3.6,expression(paste("Samples 1-17")),pos=2,cex=1,col=1)
text(21.2,3.6,expression(paste("Samples 19-78")),pos=2,cex=1,col=1)
