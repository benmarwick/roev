...
dev.size('in')
#============================================================ Load file
file1<-"../Gingerich2019_AnalysisFiles_FossilStudies//9.2.23_Lich1990_Cosomys.csv"
L<-read.csv(file1,header=TRUE,sep=",",quote="\"",dec=".",fill=TRUE)
nrow(L);L[1:5,]
#------------------------------------------------------Logged matrix
LL<-matrix(nrow=nrow(L),ncol=10)
  colnames(LL)=c('Age','N1','M1','S1','N2','M2','S2','N3','M3','S3')
LL[,1]=L[,3]
for (i in c(2,5,8)){
  LL[,i]=L[,i+2]
  LL[,i+1]=log(L[,i+3])
...
nr=nrow(LL);nr					#number of rows
nn=.5*(nr-1)*nr;nn
zc=0							#zero count
nc=0
for (i in 1:(nr-1)){				#increment
  for (sr in 1:(nr-i)){			#start row
    for (cn in seq(3,9,3)){		#columns of means
      id<-numeric(length=9)
      nc=nc+1
      iyr=LL[sr+i,1]-LL[sr,1]	#interval in years
      id[1]=iyr/gt					#interval in generations
      meandiff=LL[sr,cn]-LL[sr+i,cn]		#mean diff.
      if (!is.na(meandiff)){if (meandiff==0){zc=zc+1}}
      psd=PoolSD(LL[sr+i,cn-1],LL[sr,cn-1],LL[sr+i,cn+1],LL[sr,cn+1])#n1,n2,sd1,sd2
...
#--------------------------------------------------------------------------
writefile<-"C://R_aaROEVchapt09//9.2.23_Lich1990_Cosomys_out.csv"
