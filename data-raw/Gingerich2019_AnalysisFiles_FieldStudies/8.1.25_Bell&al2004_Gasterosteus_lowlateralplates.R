...
##8.1.25_Bell&al2004_Gasterosteus_lowlateralplates
#==========================================================================
cat(rep("\n",50))							#clear console
...
text(xos,yos+8.0,expression(paste(italic('Gasterosteus aculeatus')*
	': low-morph lateral plates')),pos=4,cex=1.3,col=1)
...
#============================================================ Load file
file1<-"C://R_aaROEVchapt08//8.1.25_Bell&al2004_Gasterosteus_lowlateralplates.csv"
...
for (i in 1:nrow(LB)){
  lines(xos-.04+c((LB[i,1]+1),(LB[i,1]+1))*xfm,
    10*c(LB[i,3]-LB[i,4],LB[i,3]+LB[i,4])-5,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:nrow(LB)){
  lines(xos+c((LB[i,1]),(LB[i,1]+1))*xfm,10*c(LB[i-1,3],LB[i,3])-5,
	lty=1,lwd=1,col=1)
}
for (i in 1:nrow(LB)){
  points(xos+(LB[i,1]+1)*xfm,10*LB[i,3]-5,pch=19,col=1)
}
...
writefile<-"C://R_aaROEVchapt08//8.1.25_Bell&al2004_Gasterosteus_lowlateralplates_out.csv"
...
text(0,6.5,"Lateral plate count",pos=4,cex=1.3,col=1)
text(12,6.5,"Lateral plate count",pos=4,cex=1.3,col=1)
