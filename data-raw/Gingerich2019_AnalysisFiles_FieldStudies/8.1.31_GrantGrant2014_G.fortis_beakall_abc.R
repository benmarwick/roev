...
##8.1.31_GrantGrant2014_G.fortis_beakall_abc
#==========================================================================
cat(rep("\n",50))							#clear console
...
xfs=0;xfe=40;yfs=2.0;yfe=2.5			#x,y foreground start and end
...
for (i in seq(yos,yoe-.5,.8)){
  text(xos,i,format(round(.5/8*(i-9)+2,digits=2),nsmall=2),pos=2,cex=1,col=1)
}
#-----------------------------------------------
...
#-----------------------------------------------
text(xos,yos+8.0,expression(paste(italic('Geospiza fortis')*
...
text(xos+7.5,yos-1.2,'Year',
	cex=1.3,col=1)
#Gray bars are drought years taken from fig. 4.12 in Grant-Grant 2014
...
#============================================================ Load file
file1<-"C://R_aaROEVchapt08//8.1.31_GrantGrant2014_G.fortis_beakall.csv"
G<-read.csv(file1,header=TRUE,sep=",",quote="\"",dec=".",fill=TRUE);G[1:3,]
GL<-matrix(nrow=40,ncol=8)
  colnames(GL)=c('Gen.','N','LnB.lx','LnB.lsd','LnB.dx','LnB.dsd',
	'LnB.wx','LnB.wsd')
...
#------------------------------------------------Plot beak length
for (i in 1:40){
  lines(xos-.04+c(GL[i,1],GL[i,1])*xfm,
    16*c(GL[i,3]-GL[i,4],GL[i,3]+GL[i,4])-23,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:40){
  lines(xos+c(GL[i-1,1],GL[i,1])*xfm,16*c(GL[i-1,3],GL[i,3])-23,
	lty=1,lwd=1,col=1)
}
for (i in 1:40){
  points(xos+GL[i,1]*xfm,16*GL[i,3]-23,pch=19,col=1)
}
#------------------------------------------------Plot beak depth
for (i in 1:40){
  lines(xos+.04+c(GL[i,1],GL[i,1])*xfm,
    16*c(GL[i,5]-GL[i,6],GL[i,5]+GL[i,6])-23,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:40){
  lines(xos+c(GL[i-1,1],GL[i,1])*xfm,16*c(GL[i-1,5],GL[i,5])-23,
	lty=1,lwd=1,col=1)
}
for (i in 1:40){
  points(xos+GL[i,1]*xfm,16*GL[i,5]-23,pch=19,col=1)
}
#------------------------------------------------Plot beak width
for (i in 1:40){
  lines(xos-.04+c(GL[i,1],GL[i,1])*xfm,
    16*c(GL[i,7]-GL[i,8],GL[i,7]+GL[i,8])-23,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:40){
  lines(xos+c(GL[i-1,1],GL[i,1])*xfm,16*c(GL[i-1,7],GL[i,7])-23,
	lty=1,lwd=1,col=1)
}
for (i in 1:40){
  points(xos+GL[i,1]*xfm,16*GL[i,7]-23,pch=19,col=1)
}
#------------------------------------------------Superimpose points
for (i in 1:40){
  points(xos+GL[i,1]*xfm,16*GL[i,3]-23,pch=19,col=1)
...
writefile<-"C://R_aaROEVchapt08//8.1.31_GrantGrant2014_G.fortis_beakall_out.csv"
...
bootresultd=TriPanelBC(idrcL,"r",1,4.5,bootn,mode,psize,"lower")
wrlb.d=bootresultd[[1]];bootmat.d=bootresultd[[2]]
proc.time()-ptm
#========================================================== Plot LRI panel c
#send idrx matrix, n, mode(diff/rate), panel placement coordinates, mode
bootresultr=TriPanelBC(idrcD,"r",13,4.5,bootn,mode,psize,"lower")
...
text(0,6.5,"Beak length",pos=4,cex=1.3,col=1)
text(12,6.5,"Beak depth",pos=4,cex=1.3,col=1)
