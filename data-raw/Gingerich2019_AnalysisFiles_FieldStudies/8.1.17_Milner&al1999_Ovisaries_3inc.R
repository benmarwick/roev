...
##8.1.17_Milner&al1999_Ovisaries_3inc
#==========================================================================
cat(rep("\n",50))							#clear console
...
xfs=0;xfe=9;yfs=5;yfe=5.4			#x,y foreground start and end
...
for (i in seq(yos,yoe-.5,1)){
  text(xos,i,format(round(.05*(i-7)+4.9,digits=2),nsmall=2),pos=2,cex=1,col=1)
}
for (i in xos+c(xfs:xfe)*1*xfm){
  lines(c(i,i),c(yos,yos-.1),lwd=1,lty=1,col=1)
}
for (i in c(1988,1990,1992,1994,1996)){
  text(xos+(i-1988+1)*xfm,yos,i,pos=1,cex=1,col=1)
}
text(xos,yos+8.0,expression(paste(italic('Ovis aries')*
	': incisive tooth width')),pos=4,cex=1.3,col=1)
text(xos-1.9,yos+4,'Ln width (mm)',
	srt=90,cex=1.3,col=1)
text(xos+7,yos-1.2,'Year',
	cex=1.3,col=1)
#============================================================ Load file
file1<-"C://R_aaROEVchapt08//8.1.17_Milner&al1999_Ovisaries_3inc.csv"
M<-read.csv(file1,header=TRUE,sep=",",quote="\"",dec=".",fill=TRUE);M
#----------------------------Fill MF female matrix for 'all' and 'survivors'
MF<-matrix(nrow=6,ncol=8); rownames(MF)=c('Na','Ma','Va','Ns','Ms','Vs')
  colnames(MF)=as.character(seq(1989,1996))
for (i in 1:8){
...
MM<-matrix(nrow=6,ncol=8); rownames(MM)=c('Na','Ma','Va','Ns','Ms','Vs')
  colnames(MM)=as.character(seq(1989,1996))
for (i in 2:8){
...
#----------------------------------------------------------------------
ML<-matrix(nrow=8,ncol=7)
  colnames(ML)=c('Gen.','Nf','Ln.fx','Ln.fsd','Nm','Ln.mx','Ln.msd')
for (i in 1:length(ML[,1])){
  ML[i,1]=i
  ML[i,2]=MF[i,1]
  ML[i,3]=log(MF[i,2])
  ML[i,4]=sqrt(MF[i,3])/MF[i,2]
  ML[i,5]=MM[i,1]
  ML[i,6]=log(MM[i,2])
  ML[i,7]=sqrt(MM[i,3])/MM[i,2]
}
ML[c(1:2,(length(ML[,1])-1):length(ML[,1])),]
#------------------------------------------------Plot male ln weights
for (i in seq(yos,yoe-.5,1)){
  lines(xos+c(0,9)*xfm,c(i,i),lty=2,lwd=1,col=gray(6/10))
}
for (i in 2:8){
  lines(xos+.03+c(ML[i,1]+1,ML[i,1]+1)*xfm,
    20*c(ML[i,6]-ML[i,7],ML[i,6]+ML[i,7])-52,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 3:8){
  lines(xos+c(ML[i-1,1]+1,ML[i,1]+1)*xfm,20*c(ML[i-1,6],ML[i,6])-52,
	lty=1,lwd=1,col=1)
}
for (i in 2:8){
  points(xos+(ML[i,1]+1)*xfm,20*ML[i,6]-52,pch=19,cex=1.1,col=1)
}
#------------------------------------------------Plot female ln weights
for (i in 1:8){
  lines(xos-.03+c(ML[i,1]+1,ML[i,1]+1)*xfm,
    20*c(ML[i,3]-ML[i,4],ML[i,3]+ML[i,4])-52,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:8){
  lines(xos+c(ML[i-1,1]+1,ML[i,1]+1)*xfm,20*c(ML[i-1,3],ML[i,3])-52,
	lty=1,lwd=1,col=1)
}
for (i in 1:8){
  points(xos+(ML[i,1]+1)*xfm,20*ML[i,3]-52,pch=21,cex=1.2,bg='white',col=1)
}
...
sum(row.has.na)					#rows with MA
MLred=ML[!row.has.na,]				#reduced MA with NA rows removed
MLred	
n=nrow(MLred);n
nn=.5*(n-1)*n;nn
idr=matrix(nrow=nn,ncol=9)
...
for (k in 1:(n-1)){    #run length
  for (i in 1:(n-k)){  #starting position
    nc=nc+1
    if(k<=gentime){idr[nc,1]=1} else {idr[nc,1]=k/gentime}	#interval
    meandiff=MLred[(i+k),6]-MLred[i,6]					#mean diff.
    poolsd=PoolSD(MLred[i+k,5],MLred[i,5],
       MLred[i+k,7],MLred[i,7])					#n1,n2,sd1,sd2
    idr[nc,2]=meandiff/poolsd						#diff.sd
...
writefile<-"C://R_aaROEVchapt08//8.1.17_Milner&al1999_Ovisaries_3inc_out.csv"
...
text(0,6.5,"Female incisive width",pos=4,cex=1.3,col=1)
text(12,6.5,"Male incisive width",pos=4,cex=1.3,col=1)
