...
##8.1.17_Milner&al1999_Ovisaries_2leg
#==========================================================================
cat(rep("\n",50))							#clear console
...
text(xos,yos+8.0,expression(paste(italic('Ovis aries')*
	': adult leg length')),pos=4,cex=1.3,col=1)
text(xos-1.9,yos+4,'Ln length (mm)',
	srt=90,cex=1.3,col=1)
text(xos+7,yos-1.2,'Year',
	cex=1.3,col=1)
#============================================================ Load file
file1<-"C://R_aaROEVchapt08//8.1.17_Milner&al1999_Ovisaries_2leg.csv"
M<-read.csv(file1,header=TRUE,sep=",",quote="\"",dec=".",fill=TRUE);M
#----------------------------Fill MF female matrix for 'all' and 'survivors'
MF<-matrix(nrow=6,ncol=9); rownames(MF)=c('Na','Ma','Va','Ns','Ms','Vs')
  colnames(MF)=as.character(seq(1988,1996))
for (i in 1:9){
...
MM<-matrix(nrow=6,ncol=9); rownames(MM)=c('Na','Ma','Va','Ns','Ms','Vs')
...
for (i in 1:9){
  lines(xos+.03+c(ML[i,1],ML[i,1])*xfm,
    20*c(ML[i,6]-ML[i,7],ML[i,6]+ML[i,7])-91,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:9){
  lines(xos+c(ML[i-1,1],ML[i,1])*xfm,20*c(ML[i-1,6],ML[i,6])-91,
	lty=1,lwd=1,col=1)
}
for (i in 1:9){
  points(xos+ML[i,1]*xfm,20*ML[i,6]-91,pch=19,cex=1.1,col=1)
}
#------------------------------------------------Plot female ln weights
for (i in 1:9){
  lines(xos-.03+c(ML[i,1],ML[i,1])*xfm,
    20*c(ML[i,3]-ML[i,4],ML[i,3]+ML[i,4])-91,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:9){
  lines(xos+c(ML[i-1,1],ML[i,1])*xfm,20*c(ML[i-1,3],ML[i,3])-91,
	lty=1,lwd=1,col=1)
}
for (i in 1:9){
  points(xos+(ML[i,1])*xfm,20*ML[i,3]-91,pch=21,cex=1.2,bg='white',col=1)
}
...
writefile<-"C://R_aaROEVchapt08//8.1.17_Milner&al1999_Ovisaries_2leg_out.csv"
...
text(0,6.5,"Female leg length",pos=4,cex=1.3,col=1)
text(12,6.5,"Male leg length",pos=4,cex=1.3,col=1)
