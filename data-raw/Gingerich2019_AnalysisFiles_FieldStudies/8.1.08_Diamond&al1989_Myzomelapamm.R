...
##8.1.08_Diamond&al1989_Myzomelapamm
#==========================================================================
cat(rep("\n",50))							#clear console
...
#============================================================ Load file
file1<-"C://R_aaROEVchapt08//8.1.08_Diamond&al1989_Myzomelapamm.csv"
D<-read.csv(file1,header=TRUE,sep=",",quote="\"",dec=".",fill=TRUE);D
#----------------------------------------------------------------------
D[1,]
LS<-matrix(nrow=nrow(D),ncol=3*6)		#Ln matrix for sclateri
...
AS<-matrix(nrow=2,ncol=3*6)	#Ln matrix for aveaged non-Long pammelaena
  colnames(LS)=c('N1','M1','S1','N2','M2','S2','N3','M3','S3',
    'N4','M4','S4','N5','M5','S5','N6','M6','S6')
Nnz<-numeric(length=6)
for (tr in 1:6){  #6){	#trait number for wing, tail, bill, culmen, tarsus, wgt
  Nsum=sum(LS[seq(3,21,2),3*tr-2],na.rm=TRUE)		#male line
  Nnz[tr]=sum(LS[seq(3,21,2),3*tr-2]!=0,na.rm=TRUE)	#non-zero entries
  AS[1,3*tr-2]=Nsum
  AS[1,3*tr-1]=sum(LS[seq(3,21,2),3*tr-2]*LS[seq(3,21,2),3*tr-1],na.rm=TRUE)/
    Nsum
  AS[1,3*tr]=sqrt(sum((LS[seq(3,21,2),3*tr-2]-1)*LS[seq(3,21,2),3*tr]^2,
    na.rm=TRUE)/(Nsum-Nnz[tr]))

  Nsum=sum(LS[seq(4,22,2),3*tr-2],na.rm=TRUE)		#female line
  Nnz[tr]=sum(LS[seq(4,22,2),3*tr-2]!=0,na.rm=TRUE)	#non-zero entries
  AS[2,3*tr-2]=Nsum
  AS[2,3*tr-1]=sum(LS[seq(4,22,2),3*tr-2]*LS[seq(4,22,2),3*tr-1],na.rm=TRUE)/
    Nsum
  AS[2,3*tr]=sqrt(sum((LS[seq(4,22,2),3*tr-2]-1)*LS[seq(4,22,2),3*tr]^2,
    na.rm=TRUE)/(Nsum-Nnz[tr]))
}
AS
#============================================================== Rate calc 
...
writefile<-"C://R_aaROEVchapt08//8.1.08_Diamond&al1989_Myzomelapamm_out.csv"
