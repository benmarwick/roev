...
##8.1.15_Larsson&al1998_Branta_tars
#==========================================================================
cat(rep("\n",50))							#clear console
...
M
#-------------------------------------------------Head length matrix
TL<-matrix(nrow=nrow(M),ncol=7)
  colnames(TL)=c('Gen.','Nf','Ln.fx','Ln.fsd','Nm','Ln.mx','Ln.msd')
...
TL[c(1:2,(length(TL[,1])-1):length(TL[,1])),]
#------------------------------------------------Plot male tarsus lengths
for (i in 1:nrow(TL)){
  lines(xos+.03+c(TL[i,1],TL[i,1])*xfm,
    40*c(TL[i,6]-TL[i,7],TL[i,6]+TL[i,7])-164,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:nrow(TL)){
  lines(xos+c(TL[i-1,1],TL[i,1])*xfm,40*c(TL[i-1,6],TL[i,6])-164,
	lty=1,lwd=1,col=1)
}
for (i in 1:nrow(TL)){
  points(xos+(TL[i,1])*xfm,40*TL[i,6]-164,pch=19,cex=1.1,col=1)
}
#------------------------------------------------Plot female tarsus lengths
for (i in 1:nrow(TL)){
  lines(xos-.03+c(TL[i,1],TL[i,1])*xfm,
    40*c(TL[i,3]-TL[i,4],TL[i,3]+TL[i,4])-164,
    lty=1,lwd=1,col=gray(6/10))
}
for (i in 2:nrow(TL)){
  lines(xos+c(TL[i-1,1],TL[i,1])*xfm,40*c(TL[i-1,3],TL[i,3])-164,
	lty=1,lwd=1,col=1)
}
for (i in 1:nrow(TL)){
  points(xos+(TL[i,1])*xfm,40*TL[i,3]-164,pch=21,cex=1.2,bg='white',col=1)
}
#============================================= Rate calc: female tarsus lengths
...
for (k in 1:(n-1)){	#run length
  for (i in 1:(n-k)){  						#starting position
    ...
    meandiff=TL[(i+k),3]-TL[i,3]					#mean diff.
    poolsd=PoolSD(TL[i+k,2],TL[i,2],TL[i+k,4],TL[i,4])	#n1,n2,sd1,sd2
...
for (k in 1:(n-1)){    #run length
  for (i in 1:(n-k)){  #starting position
    nc=nc+1
    if(k<=gentime){idr[nc,1]=1} else {idr[nc,1]=k/gentime}	#interval
    meandiff=TL[(i+k),6]-TL[i,6]					#mean diff.
    poolsd=PoolSD(TL[i+k,5],TL[i,5],TL[i+k,7],TL[i,7])	#n1,n2,sd1,sd2
    idr[nc,2]=meandiff/poolsd						#diff.sd
...
idrsum
nrow(idrxA)
writefile<-"C://R_aaROEVchapt08//8.1.15_Larsson&al1998_Branta_tars_out.csv"
...
text(0,6.5,"Female tarsus length",pos=4,cex=1.3,col=1)
text(12,6.5,"Male tarsus length",pos=4,cex=1.3,col=1)
